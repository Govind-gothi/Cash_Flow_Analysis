...
def css_check():
    import os
    # Asli rasta pata karte hain
    current_dir = os.path.dirname(os.path.abspath(__file__))
    css_path = os.path.join(current_dir, 'static', 'css', 'style.css')
    
    if os.path.exists(css_path):
        return f"<h1>✅ CSS FILE MIL GAYI!</h1><p>Path: {css_path}</p>"
    else:
        return f"<h1>❌ CSS FILE NAHI MILI!</h1><p>Flask isko yahan dhund raha hai: {css_path}</p>"
